...
class DecisionTree:
    ...
    def _grow_tree(self, X, y, depth=0):
        ...
        feat_idxs = np.random.choice(n_features, self.n_feats, replace=True)

        # greedly slect the best according to information gain
    ...
    def _information_gain(self, y, X_column, split_thresh):
        ...
        # compute the weighted avg, of the loss for the childre
        n = len(y)
        n_l, n_r = len(left_idx), len(right_idx)
        e_l, e_r = entropy(y[left_idx]), entropy(y[right_idx])
        child_entropy = (n_l / n) * e_l + (n_r / n)  * e_r

        #information gain is the difference in loss before vs. after
